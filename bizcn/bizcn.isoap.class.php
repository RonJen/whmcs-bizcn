<?php
$GLOBALS['_transient']['static']['bizcnsoap_base']['globalDebugLevel'] = 9;
class bizcnsoap_base{
    var $title = 'BizcnSOAP';
    var $version = '0.9.5-Use NuSOAP';
    ...
    function bizcnsoap_base(){
        $this -> debugLevel = $GLOBALS['_transient']['static']['bizcnsoap_base']['globalDebugLevel'];
    }
    function getGlobalDebugLevel(){
        return $GLOBALS['_transient']['static']['bizcnsoap_base']['globalDebugLevel'];
    }
    function setGlobalDebugLevel($level){
        $GLOBALS['_transient']['static']['bizcnsoap_base']['globalDebugLevel'] = $level;
    }
    ...
    function serialize_val($val, $name = false, $type = false, $name_ns = false, $type_ns = false, $attributes = false, $use = 'encoded', $soapval = false){
        ...
        $xml = '';
        switch(true){
        case (is_bool($val) || $type == 'boolean'): $this -> debug("serialize_val: serialize boolean");
            if ($type == 'boolean'){
                $val = $val ? 'true' : 'false';
            }elseif (! $val){
                $val = 0;
            ...
            break;
        case (is_int($val) || is_long($val) || $type == 'int'): $this -> debug("serialize_val: serialize int");
            if ($use == 'literal'){
                $xml .= "<$name$xmlns$atts>$val</$name>";
            }else{
            ...
            break;
        case (is_float($val) || is_double($val) || $type == 'float'): $this -> debug("serialize_val: serialize float");
            if ($use == 'literal'){
                $xml .= "<$name$xmlns$atts>$val</$name>";
            }else{
            ...
            break;
        case (is_string($val) || $type == 'string'): $this -> debug("serialize_val: serialize string");
            $val = $this -> expandEntities($val);
            if ($use == 'literal'){
                $xml .= "<$name$xmlns$atts>$val</$name>";
            }else{
            ...
            break;
        case is_object($val): $this -> debug("serialize_val: serialize object");
            ...
            break;
            break;
        case (is_array($val) || $type): $valueType = $this -> isArraySimpleOrStruct($val);
            ...
            break;
        default: $this -> debug("serialize_val: serialize unknown");
...
?><?php
 class bizcnsoap_fault extends bizcnsoap_base{
    var $faultcode;
    var $faultactor;
    var $faultstring;
    var $faultdetail;
    function bizcnsoap_fault($faultcode, $faultactor = '', $faultstring = '', $faultdetail = ''){
        parent :: bizcnsoap_base();
...
class soap_fault extends bizcnsoap_fault{
}
?><?php
 class bizcnsoap_xmlschema extends bizcnsoap_base{
    var $schema = '';
    var $xml = '';
    ...
    function bizcnsoap_xmlschema($schema = '', $xml = '', $namespaces = array()){
        parent :: bizcnsoap_base();
        $this -> debug('bizcnsoap_xmlschema class instantiated, inside constructor');
        $this -> schema = $schema;
        $this -> xml = $xml;
    ...
    function schemaStartElement($parser, $name, $attrs){
        ...
        switch($name){
        case 'all': case 'choice': case 'group': case 'sequence': $this -> complexTypes[$this -> currentComplexType]['compositor'] = $name;
            break;
        ...
        case 'complexContent': $this -> xdebug("do nothing for element $name");
            break;
        case 'complexType': array_push($this -> complexTypeStack, $this -> currentComplexType);
            if(isset($attrs['name'])){
            ...
            break;
        case 'element': array_push($this -> elementStack, $this -> currentElement);
            if (!isset($attrs['form'])){
                if ($this -> currentComplexType){
        ...
        case 'extension': $this -> xdebug('extension ' . $attrs['base']);
            ...
            break;
        case 'import': if (isset($attrs['schemaLocation'])){
            ...
            break;
        case 'include': if (isset($attrs['schemaLocation'])){
        ...
        case 'restriction': $this -> xdebug('restriction ' . $attrs['base']);
            ...
            break;
        case 'schema': $this -> schemaInfo = $attrs;
        ...
        case 'simpleContent': if ($this -> currentComplexType){
            ...
            break;
        case 'simpleType': array_push($this -> simpleTypeStack, $this -> currentSimpleType);
            if(isset($attrs['name'])){
        ...
        case 'union': $this -> xdebug("do nothing for element $name");
            break;
        default: $this -> xdebug("do not have any logic to process element $name");
        }
    }
...
class XMLSchema extends bizcnsoap_xmlschema{
}
?><?php
 class soapval extends bizcnsoap_base{
    var $name;
    var $type;
    var $value;
    var $element_ns;
...
?><?php
 class soap_transport_http extends bizcnsoap_base{
    var $url = '';
    var $uri = '';
    var $digest_uri = '';
    var $scheme = '';
...
?><?php
 class bizcnsoap_server extends bizcnsoap_base{
    var $headers = array();
    var $request = '';
    var $requestHeaders = '';
    ...
    function bizcnsoap_server($wsdl = false){
        parent :: bizcnsoap_base();
        global $debug;
        ...
        if (isset($debug)){
            $this -> debug("In bizcnsoap_server, set debug_flag=$debug based on global flag");
            $this -> debug_flag = $debug;
        }elseif (isset($_SERVER['QUERY_STRING'])){
            $qs = explode('&', $_SERVER['QUERY_STRING']);
            foreach ($qs as $v){
                if (substr($v, 0, 6) == 'debug='){
                    $this -> debug("In bizcnsoap_server, set debug_flag=" . substr($v, 6) . " based on query string #1");
            ...
            foreach ($qs as $v){
                if (substr($v, 0, 6) == 'debug='){
                    $this -> debug("In bizcnsoap_server, set debug_flag=" . substr($v, 6) . " based on query string #2");
                    $this -> debug_flag = substr($v, 6);
                }
            }
        }
        if($wsdl){
            $this -> debug("In bizcnsoap_server, WSDL is specified");
            ...
            if ($this -> methodparams){
                foreach ($this -> methodparams as $param){
                    if (is_array($param) || is_object($param)){
                        $this -> fault('SOAP-ENV:Client', 'BizcnSOAP does not handle complexType parameters correctly when using eval; call_user_func_array must be available');
                        return;
                    }
                    $funcCall .= "\"$param\",";
                }
    ...
    function serialize_return(){
        ...
        if (isset($this -> methodreturn) && is_object($this -> methodreturn) && ((get_class($this -> methodreturn) == 'soap_fault') || (get_class($this -> methodreturn) == 'bizcnsoap_fault'))){
    ...
    function parseRequest($headers, $data){
        ...
        $this -> debug('Use encoding: ' . $this -> xml_encoding . ' when creating bizcnsoap_parser');
        $parser = new bizcnsoap_parser($data, $this -> xml_encoding, '', $this -> decode_utf8);
    ...
    function fault($faultcode, $faultstring, $faultactor = '', $faultdetail = ''){
        ...
        $this -> fault = new bizcnsoap_fault($faultcode, $faultactor, $faultstring, $faultdetail);
    ...
    function configureWSDL($serviceName, $namespace = false, $endpoint = false, $style = 'rpc', $transport = 'http://schemas.xmlsoap.org/soap/http', $schemaTargetNamespace = false){
        ...
        $this -> wsdl -> schemas[$schemaTargetNamespace][0] = new bizcnsoap_xmlschema('', '', $this -> wsdl -> namespaces);
        if ($style == 'document'){
...
class soap_server extends bizcnsoap_server{
}
?><?php
 class wsdl extends bizcnsoap_base{
    var $wsdl;
    var $schemas = array();
    var $currentSchema;
    var $message = array();
    ...
    var $curl_options = array("CURLOPT_SSL_VERIFYPEER" => false);
    var $use_curl = false;
    var $username = '';
    var $password = '';
    var $authtype = '';
    ...
    function start_element($parser, $name, $attrs){
        if ($this -> status == 'schema'){
            ...
            $this -> status = 'schema';
            $this -> currentSchema = new bizcnsoap_xmlschema('', '', $this -> namespaces);
            $this -> currentSchema -> schemaStartElement($parser, $name, $attrs);
            $this -> appendDebug($this -> currentSchema -> getDebug());
            $this -> currentSchema -> clearDebug();
        }else{
            ...
            case 'message': if ($name == 'part'){
                ...
                break;
            case 'portType': switch ($name){
                case 'operation': $this -> currentPortOperation = $attrs['name'];
                    $this -> debug("portType $this->currentPortType operation: $this->currentPortOperation");
                    if (isset($attrs['parameterOrder'])){
                        $this -> portTypes[$this -> currentPortType][$attrs['name']]['parameterOrder'] = $attrs['parameterOrder'];
                    }
                    break;
                case 'documentation': $this -> documentation = true;
                    break;
                default: $m = isset($attrs['message']) ? $this -> getLocalPart($attrs['message']) : '';
                    $this -> portTypes[$this -> currentPortType][$this -> currentPortOperation][$name]['message'] = $m;
                    break;
                }
                break;
                case 'binding': switch ($name){
                    case 'binding': if (isset($attrs['style'])){
                            $this -> bindings[$this -> currentBinding]['prefix'] = $prefix;
                        }
                        $this -> bindings[$this -> currentBinding] = array_merge($this -> bindings[$this -> currentBinding], $attrs);
                        break;
                    case 'header': $this -> bindings[$this -> currentBinding]['operations'][$this -> currentOperation][$this -> opStatus]['headers'][] = $attrs;
                        break;
                    case 'operation': if (isset($attrs['soapAction'])){
                        ...
                        break;
                    case 'input': $this -> opStatus = 'input';
                        break;
                    case 'output': $this -> opStatus = 'output';
                        break;
                    case 'body': if (isset($this -> bindings[$this -> currentBinding]['operations'][$this -> currentOperation][$this -> opStatus])){
                    ...
                    case 'service': switch ($name){
                        case 'port': $this -> currentPort = $attrs['name'];
                            $this -> debug('current port: ' . $this -> currentPort);
                            $this -> ports[$this -> currentPort]['binding'] = $this -> getLocalPart($attrs['binding']);
                            break;
                        case 'address': $this -> ports[$this -> currentPort]['location'] = $attrs['location'];
                    ...
                    switch ($name){
                    case 'import': if (isset($attrs['location'])){
                        ...
                        break;
                    case 'message': $this -> status = 'message';
                        $this -> messages[$attrs['name']] = array();
                        $this -> currentMessage = $attrs['name'];
                        break;
                    case 'portType': $this -> status = 'portType';
                        $this -> portTypes[$attrs['name']] = array();
                        $this -> currentPortType = $attrs['name'];
                        break;
                    case "binding": if (isset($attrs['name'])){
                        ...
                        break;
                    case 'service': $this -> serviceName = $attrs['name'];
                        $this -> status = 'service';
                        $this -> debug('current service: ' . $this -> serviceName);
                        break;
                    case 'definitions': foreach ($attrs as $name => $value){
    ...
    function webDescription(){
        ...
        $b = '
		<html><head><title>BizcnSOAP: ' . $this -> serviceName . '</title>
		<style type="text/css">
...
?><?php
 class bizcnsoap_parser extends bizcnsoap_base{
    var $xml = '';
    var $xml_encoding = '';
    var $method = '';
    var $root_struct = '';
    ...
    var $multirefs = array();
    var $decode_utf8 = true;
    function bizcnsoap_parser($xml, $encoding = 'UTF-8', $method = '', $decode_utf8 = true){
        parent :: bizcnsoap_base();
        $this -> xml = $xml;
        ...
        if(!empty($xml)){
            ...
            $this -> debug('Entering bizcnsoap_parser(), length=' . strlen($xml) . ', encoding=' . $encoding);
            ...
            if(!xml_parse($this -> parser, $xml, true)){
                ...
                $this -> debug('in bizcnsoap_parser ctor, message:');
...
class soap_parser extends bizcnsoap_parser{
}
?><?php
 class bizcnsoap_client extends bizcnsoap_base{
    var $username = '';
    var $password = '';
    var $authtype = '';
    var $certRequest = array();
    ...
    var $faultstring;
    var $faultdetail;
    function bizcnsoap_client($endpoint, $wsdl = false, $proxyhost = false, $proxyport = false, $proxyusername = false, $proxypassword = false, $timeout = 0, $response_timeout = 30, $portName = ''){
        parent :: bizcnsoap_base();
    ...
    function send($msg, $soapaction = '', $timeout = 0, $response_timeout = 30){
        $this -> checkCookies();
        switch(true){
        case preg_match('/^http/', $this -> endpoint): $this -> debug('transporting via HTTP');
            ...
            break;
        default: $this -> setError('no transport found, or selected transport is not yet supported!');
            return false;
            break;
        }
    }
    function parseResponse($headers, $data){
        ...
        $this -> debug('Use encoding: ' . $this -> xml_encoding . ' when creating bizcnsoap_parser');
        $parser = new bizcnsoap_parser($data, $this -> xml_encoding, $this -> operation, $this -> decode_utf8);
    ...
    function getProxy(){
        ...
        eval($evalStr);
        eval("\$proxy = new bizcnsoap_proxy_$r('');");
	...
	";
        ...
        $evalStr = 'class bizcnsoap_proxy_' . $r . ' extends bizcnsoap_client {
	' . $evalStr . '
}';
        return $evalStr;
    }
...
if (!extension_loaded('soap')){
    class soapclient extends bizcnsoap_client{
    }
}
?>
